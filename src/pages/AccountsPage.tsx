...
function ChipIcon({ isBlack }: { isBlack: boolean }) {
	return (
		<div
			className={`h-8 w-10 rounded-md ${isBlack ? "bg-foreground/30" : "bg-foreground/20"} flex items-center justify-center`}
		>
			<div
				className={`h-5 w-7 rounded-xs border ${isBlack ? "border-foreground/20" : "border-foreground/15"} grid grid-cols-3 grid-rows-2 gap-px p-px`}
			>
				{Array.from({ length: 6 }).map((_, i) => (
					<div
						key={i}
						className={`${isBlack ? "bg-foreground/15" : "bg-foreground/10"} rounded-[1px]`}
					/>
				))}
			</div>
		</div>
		...
		<div className="space-y-4">
			...
			<div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
				{accountList.map((acc) => (
					<div
						key={acc.name}
						className="h-full flex flex-col rounded-[var(--radius-card)] bg-muted p-5"
					>
						...
						<h2 className="text-xl md:text-2xl font-semibold text-foreground font-display tracking-tight font-mono-num">
			...
			<div className="flex gap-3">
				<button
					type="button"
					className="flex items-center gap-2 rounded-[var(--radius-widget)] bg-primary px-4 py-2.5 text-sm font-medium text-primary-foreground"
				>
					<Plus className="h-4 w-4" strokeWidth={1.5} /> {t("pages.accounts.addMoney")}
				</button>
				<button
					type="button"
					className="flex items-center gap-2 rounded-[var(--radius-widget)] bg-secondary px-4 py-2.5 text-sm font-medium text-foreground"
				>
			...
			<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
				{cards.map((card) => (
					<div
						key={card.name}
						className="h-full flex flex-col rounded-[var(--radius-card)] bg-muted p-4"
					>
						...
						<div className="rounded-[var(--radius-widget)] border border-border bg-card p-3">
							...
							<p className="text-[11px] text-muted-foreground mt-1.5">
								<span className="font-mono-num">${card.balance.toLocaleString()}</span> /{" "}
								<span className="font-mono-num">${card.limit.toLocaleString()}</span>{" "}
						...
						{activityList.map((item, i) => (
							<div key={i} className="flex items-center justify-between py-1">
								<div className="flex items-center gap-3">
									<div
										className={`flex h-8 w-8 items-center justify-center rounded-lg ${item.direction === "positive" ? "bg-muted" : "bg-muted"}`}
									>
								...
								<span
									className={`text-sm font-medium font-mono-num ${item.direction === "positive" ? "text-success" : "text-foreground"}`}
								>
			...
			<div className="flex items-center justify-between">
				...
				<button
					type="button"
					className="flex items-center gap-2 rounded-[var(--radius-widget)] bg-secondary px-3 py-2 text-sm text-muted-foreground hover:text-foreground"
				>
			...
			{/* Desktop table */}
			<div className="h-full flex flex-col rounded-[var(--radius-card)] bg-muted overflow-hidden hidden md:block">
							...
							{records.map((tx) => (
								<tr key={tx.id} className="hover:bg-accent/50 transition-colors">
									<td className="px-5 py-3">
										<div className="flex items-center gap-3">
											<div
												className={`flex h-7 w-7 items-center justify-center rounded-md ${tx.direction === "positive" ? "bg-muted" : "bg-muted"}`}
											>
									...
									<td
										className={`py-3 text-sm font-medium text-right font-mono-num ${tx.direction === "positive" ? "text-success" : "text-foreground"}`}
									>
										{tx.formattedAmount}
									</td>
									<td className="py-3 pr-5 text-right">
										<span
											className={`text-[10px] px-2 py-0.5 rounded-full ${tx.statusVariant === "success" ? "bg-muted text-success" : "bg-muted text-muted-foreground"}`}
										>
											{tx.status}
										</span>
									</td>
			...
			<div className="flex flex-col gap-2 md:hidden">
				{records.map((tx) => (
					...
					>
						<div className="flex items-center gap-3">
							<div
								className={`flex h-8 w-8 items-center justify-center rounded-lg shrink-0 bg-muted`}
							>
								{tx.direction === "positive" ? (
									<ArrowDownLeft className="h-3.5 w-3.5 text-success" strokeWidth={1.5} />
								) : (
							...
							<div className="text-right shrink-0">
								<p
									className={`text-sm font-medium font-mono-num ${tx.direction === "positive" ? "text-success" : "text-foreground"}`}
								>
									{tx.formattedAmount}
								</p>
								<span
									className={`text-[10px] ${tx.statusVariant === "success" ? "text-success" : "text-muted-foreground"}`}
								>
									{tx.status}
								</span>
							</div>

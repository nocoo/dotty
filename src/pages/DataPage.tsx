...
const ICON_PILLS = [
	...
	},
	{
		...
		tKey: "atRisk",
		icon: AlertTriangle,
		className: "bg-chart-3/20 text-chart-2",
	},
		...
		<div className="space-y-4">
			...
			<div className="h-full flex flex-col rounded-[var(--radius-card)] bg-muted p-5 md:p-6">
				<div className="space-y-2">
					...
					<p className="text-sm text-muted-foreground max-w-2xl">{t("pages.data.description")}</p>
				</div>
			</div>
			...
			<Section title={t("pages.data.statTiles")} icon={TrendingUp}>
				<div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
					{KPI_DATA.map((kpi) => (
						<div
							key={kpi.label}
							className="rounded-[var(--radius-widget)] border border-border bg-card p-4"
						>
							<p className="text-[11px] font-medium uppercase tracking-[0.2em] text-muted-foreground mb-1">
								{t(`pages.data.${kpi.label}`)}
							</p>
							...
							<div className="flex items-center gap-1 mt-2">
								...
								{kpi.trend === "down" && (
									<TrendingDown className="h-3.5 w-3.5 text-muted-foreground" strokeWidth={1.5} />
								)}
								{kpi.trend === "flat" && (
									<Minus className="h-3.5 w-3.5 text-muted-foreground" strokeWidth={1.5} />
								)}
								<span
									className={`text-xs font-medium font-mono-num ${kpi.trend === "up" ? "text-success" : "text-muted-foreground"}`}
								>
									{kpi.change}
								</span>
						...
						<div className="flex items-center gap-2">
							<div className="relative min-w-[180px]">
								...
								<Input
									...
									className="rounded-[var(--radius-widget)] border-border bg-background pl-10 text-sm h-8"
								/>
							</div>
							<button
								type="button"
								className="flex items-center gap-2 rounded-[var(--radius-widget)] bg-background px-3 py-1.5 text-xs font-medium text-muted-foreground border border-border"
							>
				...
				<div className="space-y-4">
					...
					<div>
						...
						<div className="flex flex-wrap items-center gap-2">
							...
							<Badge className="border-transparent bg-chart-5/20 text-chart-2">
								{t("pages.data.pending")}
							</Badge>
							<Badge className="border-transparent bg-chart-1/15 text-chart-1">
								{t("pages.data.failed")}
							</Badge>

	...
	return (
		<div className="flex items-center gap-1">
			<Button
				variant="outline"
				size="icon"
				className="h-8 w-8 rounded-[var(--radius-widget)]"
...
export default function NavigationPage() {
	...
	const breadcrumbSimple = BREADCRUMB_SIMPLE_KEYS.map((key) => ({
		label: t(`pages.navigation.${key}`),
	}));
	const breadcrumbIcons = BREADCRUMB_ICONS_KEYS.map((item) => ({
		label: t(`pages.navigation.${item.key}`),
		icon: item.icon,
	}));
	const breadcrumbCard = BREADCRUMB_CARD_KEYS.map((key) => ({
		label: t(`pages.navigation.${key}`),
	}));
	const stepperSteps = STEPPER_KEYS.map((key) => t(`pages.navigation.${key}`));
	const standardTabs = STANDARD_TAB_KEYS.map((key) => ({
		key,
		label: t(`pages.navigation.${key}`),
	}));
	const underlineTabs = UNDERLINE_TAB_KEYS.map((key) => ({
		key,
		label: t(`pages.navigation.${key}`),
	}));
						...
						<div className="rounded-[var(--radius-widget)] border border-border bg-card p-4">
							<div className="flex items-center justify-between">
								<p className="text-xs text-muted-foreground">
									{t("common.showing")}{" "}
									<span className="font-medium text-foreground font-mono-num">41-50</span>{" "}
									{t("common.of")}{" "}
									<span className="font-medium text-foreground font-mono-num">200</span>{" "}
									{t("common.results")}
								</p>
								<Pagination currentPage={page2} totalPages={20} onPageChange={setPage2} />
							</div>
					...
					<div>
						...
						<div className="flex items-center gap-2 mt-4">
							<Button
								variant="outline"
								size="sm"
								className="rounded-[var(--radius-widget)]"
							...
							<Button
								size="sm"
								className="rounded-[var(--radius-widget)]"
								disabled={stepperIndex === STEPPER_KEYS.length - 1}
								onClick={() => setStepperIndex((i) => i + 1)}
							>
							...
							<Button
								variant="ghost"
								size="sm"
								className="ml-auto rounded-[var(--radius-widget)]"
								...
								<TabsContent key={tab.key} value={tab.key}>
									<div className="rounded-[var(--radius-widget)] border border-border bg-card p-4">
										<p className="text-sm text-foreground">
											{t(`pages.navigation.${tab.key}Content`)}
										</p>
										{tab.key === "overview" && (
											<p className="text-xs text-muted-foreground mt-1">
							...
							{underlineTabs.map((tab) => (
								<TabsContent key={tab.key} value={tab.key}>
									<div className="rounded-[var(--radius-widget)] border border-border bg-card p-4 mt-3">
										<p className="text-sm text-foreground">
											{t(
												`pages.navigation.showing${tab.key.charAt(0).toUpperCase() + tab.key.slice(1)}`,
											)}
										</p>
									</div>
								</TabsContent>

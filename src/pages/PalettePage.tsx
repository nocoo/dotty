...
import { CHART_COLORS, chart, chartAxis, chartNegative, chartPositive } from "@/lib/palette";
...
export default function PalettePage() {
	...
	const lineLegend: LegendItemVM[] = LINE_LEGEND_KEYS.map((l) => ({
		label: t(`pages.palette.${l.tKey}`),
		color: l.color,
	}));
	const barLegend: LegendItemVM[] = BAR_LEGEND_KEYS.map((l) => ({
		label: t(`pages.palette.${l.tKey}`),
		color: l.color,
	}));
	const areaLegend: LegendItemVM[] = AREA_LEGEND_KEYS.map((l) => ({
		label: t(`pages.palette.${l.tKey}`),
		color: l.color,
	}));
		...
		<>
			...
			<div className="grid grid-cols-1 gap-4 mt-4 lg:grid-cols-2">
							...
							<div className="mt-4 grid w-full grid-cols-3 gap-x-4 gap-y-3">
								{PIE_DATA.map((item, i) => (
									<div key={item.name} className="flex flex-col items-center gap-0.5">
										<span className="font-mono-num text-sm text-foreground">{item.value}%</span>
				...
				<Section title={t("pages.palette.groupedBarChart")} icon={BarChart3}>
					<div className="rounded-[var(--radius-widget)] border border-border bg-card p-3">
						<div role="img" aria-label={t("pages.palette.groupedBarChartAria")}>
							<PixelBarChart
								data={BAR_DATA}

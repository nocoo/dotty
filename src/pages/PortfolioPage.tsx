		...
		<div className="space-y-4">
			...
			<div className="h-full flex flex-col rounded-[var(--radius-card)] bg-muted p-4 md:p-5">
				...
				<p className="text-sm text-muted-foreground">
					Portfolio performance, asset allocation donut, and individual holdings with trend
					indicators.
				</p>
			</div>
			...
			<div className="grid grid-cols-1 gap-3 md:gap-4 sm:grid-cols-3">
				...
				<div className="h-full flex flex-col rounded-[var(--radius-card)] bg-muted p-4 md:p-5">
					...
					<h2 className="text-xl md:text-2xl font-semibold text-success font-display tracking-tight font-mono-num">
						+$342.50
					</h2>
							...
							<div className="mt-4 grid w-full grid-cols-3 gap-x-4 gap-y-3">
								{holdings.map((item, i) => (
									<div key={item.name} className="flex flex-col items-center gap-0.5">
										<span className="text-sm font-medium text-foreground font-display font-mono-num">
											{item.allocation}%
										</span>
				...
				<table className="w-full">
					...
					<tbody className="flex flex-col gap-3">
						{holdings.map((item, i) => (
							<tr
								key={item.name}
								className="flex items-center justify-between rounded-[var(--radius-widget)] border border-border bg-card p-3"
							>
								...
								<td className="text-right">
									<p className="text-sm font-medium text-foreground font-mono-num">
										${item.value.toLocaleString()}
									</p>
									<p className={`text-xs ${item.up ? "text-success" : "text-muted-foreground"}`}>
										{item.change}
									</p>
								</td>
							</tr>

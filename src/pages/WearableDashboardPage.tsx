...
const useStatCards = (t: (key: string) => string) => [
	...
	},
	{
		...
		icon: Heart,
		trend: { value: -1.4, label: "vs last week" },
	},
	{
		...
		icon: Moon,
		trend: { value: 2.6, label: "vs last week" },
	},
];
...
const sleepSlots = Array.from({ length: 24 }).map((_, i) => ({
	color:
		i < 6 ? CHART_COLORS[0] : i < 10 ? CHART_COLORS[2] : i < 16 ? CHART_COLORS[4] : CHART_COLORS[6],
	label: `Hour ${i}`,
}));
...
const heartRateSlots = Array.from({ length: 24 }).map((_, i) => ({
	color:
		i < 8 ? CHART_COLORS[4] : i < 16 ? CHART_COLORS[2] : i < 20 ? CHART_COLORS[0] : CHART_COLORS[8],
	label: `Hour ${i}`,
}));
...
function Section({
	...
	return (
		<div
			className={`h-full flex flex-col rounded-[var(--radius-card)] bg-muted p-4 md:p-5 ${className}`}
		>
		...
		<div className="space-y-4">
				...
				<div className="mt-2 flex items-center gap-3">
					...
					<div>
						<h1 className="text-lg font-semibold font-display text-foreground">
							{t("pages.wearable.title")}
						</h1>
						<p className="text-sm text-muted-foreground">{t("pages.wearable.description")}</p>
					</div>
				</div>
			</div>
			...
			<div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
				<Section
					...
					trailing={
						<span className="text-sm font-semibold font-mono-num text-foreground">7h 42m</span>
					}
				>
				...
				<Section
					...
					trailing={
						<span className="text-sm font-semibold font-mono-num text-foreground">68 bpm</span>
					}
				>
					<div className="rounded-[var(--radius-widget)] border border-border bg-card p-3">
			...
			<div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
				...
				<Section
					icon={Clock}
					title={t("pages.wearable.dailyTimeline")}
					className="lg:col-span-2 max-h-[420px] overflow-y-auto"
				>
					<div className="rounded-[var(--radius-widget)] border border-border bg-card p-3">

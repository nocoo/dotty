	...
	return (
		<div className="space-y-4">
			...
			<div className="h-full flex flex-col rounded-[var(--radius-card)] bg-muted p-4 md:p-5">
				...
				<p className="text-sm text-muted-foreground">{t("pages.components.description")}</p>
			</div>
			...
			<Section title={t("pages.components.highlights")} icon={Activity}>
				<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
					<div className="rounded-[var(--radius-widget)] border border-border bg-card p-4">
						...
						<button
							type="button"
							className="mt-3 rounded-[var(--radius-widget)] bg-secondary px-3 py-2 text-xs text-foreground"
						>
							{t("common.viewModule")}
						</button>
					</div>
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-2">
					{goals.map((goal) => {
						...
						return (
							<div
								key={goal.name}
								className="rounded-[var(--radius-widget)] border border-border bg-card p-5"
							>
								<div className="flex items-center gap-3 mb-4">
									...
									<div className="flex-1">
										...
										<p className="text-xs text-muted-foreground">
											<span className="font-mono-num">${goal.saved.toLocaleString()}</span> of{" "}
											<span className="font-mono-num">${goal.target.toLocaleString()}</span>
										</p>
									</div>
								...
								<div className="mt-3 flex items-center gap-4">
									<span className="text-xs text-muted-foreground">
										{t("pages.components.monthlyTarget")}{" "}
										<span className="font-mono-num">${goal.monthlyTarget.toLocaleString()}</span>
									</span>
									{goal.onTrack && (

...
export function PixelBarChart({
	...
	const blockSize = height ? Math.floor(height / gridRows) - blockGap : blockSizeProp;
						...
						{data.map((month, mIdx) => (
							<span
								...
								className={cn(
									...
									activeMonth === mIdx ? "text-foreground font-medium" : "text-muted-foreground",
								)}
			...
			{tooltip !== null && (
				...
				>
					<p className="text-xs font-medium text-foreground mb-1.5">
						{data[tooltip.monthIdx].label}
						{tooltipYearSuffix}
					</p>
					{data[tooltip.monthIdx].series.map((val, sIdx) => (
						<div key={sIdx} className="flex items-center gap-2 text-xs">
							<span
								className={cn("inline-block w-2 h-2", SERIES_FILLS[sIdx % SERIES_FILLS.length])}
							/>
							<span className="text-muted-foreground">{labels[sIdx]}</span>
							<span className="font-medium text-foreground ml-auto pl-3">{formatYLabel(val)}</span>
						</div>
					))}
				</div>
			)}
